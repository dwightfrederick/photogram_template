class TripsController < ApplicationController
  before_action :current_user_must_be_trip_user, :only => [:show, :edit_form, :update_row, :destroy_row]
  ...
  def index
    ...
    @trips = @q.result(:distinct => true).includes(:user).page(params[:page]).per(10)
